...
piql.data.select.MCMR <- function(pulled.PIQL.data, course = 1, numBlanks.allowed = 0) {
  ...
  if (course > 100) {
    ii = match(course, c( 121, 122, 123, 141, 142, 143, 224, 225, 226, 321, 322, 323, 325 ) )
  } else {ii = course}
  working.data <- PIQL.data[[ii]]
  ...
  # Format return
  thing.return <- list()
  thing.return$data.alpha <- data.alpha
  thing.return$data.num <- data.num
  thing.return$nS.details <- nS.details
  return(thing.return)
}
