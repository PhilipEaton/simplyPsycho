...
#'
#' @description Performs ITA using strict procedures.
...
ita.strict <- function(data, type = 1) {
  ...
  for (mm in 1:(nQ*(nQ-1))) {
    ...
    map.contradictions.cur <- map.contradictions.cur[-nrow(map.contradictions.cur),]
  }
  # Format return
  results.iita <- list()
